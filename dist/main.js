 ...
 */
function broadcastProgress(step, current, total, message, reportId) {
 ...
 */
function parseReturnConfigs(returnsData) {
...
function printSummary(results) {
    ...
    for (const result of results) {
        ...
        logger.info(`  Comparison: ${result.comparisonInstance.refDate} vs ${result.baseInstance.refDate}`);
...
async function main() {
    ...
    try {
        ...
        appLogger.info('='.repeat(80));
        broadcastProgress(STEPS.AUTHENTICATION.name, STEPS.AUTHENTICATION.index, STEPS.AUTHENTICATION.total, 'Authenticating with AgileReporter...', reportIdFromArgs // ADDED
        );
        ...
        appLogger.info('='.repeat(80));
        broadcastProgress(STEPS.ANALYZING.name, STEPS.ANALYZING.index, STEPS.ANALYZING.total, `Analyzing ${returns.length} returns...`, reportIdFromArgs // ADDED
        );
        const analyzer = new VarianceAnalyzer(client);
        // Listen to analyzer progress events
        analyzer.on('progress', (event) => {
            broadcastProgress(STEPS.ANALYZING.name, STEPS.ANALYZING.index, STEPS.ANALYZING.total, `Processing returns: ${event.current}/${event.total} - ${event.message}`, reportIdFromArgs // ADDED
            );
        });
        ...
        if (results.length > 0) {
            ...
            appLogger.info('='.repeat(80));
            broadcastProgress(STEPS.EXPORTING.name, STEPS.EXPORTING.index, STEPS.EXPORTING.total, 'Creating Excel workbook...', reportIdFromArgs // ADDED
            );
            try {
                ...
                appLogger.info('='.repeat(80));
                broadcastProgress(STEPS.SAVING.name, STEPS.SAVING.index, STEPS.SAVING.total, 'Saving report metadata and details...', reportIdFromArgs // ADDED
                );
                const reportSaver = new ReportSaver();
                await reportSaver.initialize();
                const reportId = await reportSaver.saveReport(results, configData, options.output, Date.now() - startTime, 'completed');

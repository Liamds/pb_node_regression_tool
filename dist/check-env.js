...
try {
    ...
    for (const [key, expectedValue] of Object.entries(expected)) {
        const actualValue = authConfig[key];
        ...
        if (actualValue !== expectedValue) {
            ...
            for (let i = 0; i < Math.min(expectedValue.length, String(actualValue).length); i++) {
                if (expectedValue[i] !== String(actualValue)[i]) {
                    console.log(`  First diff at position ${i}: expected '${expectedValue[i]}' (code=${expectedValue.charCodeAt(i)}), got '${String(actualValue)[i]}' (code=${String(actualValue).charCodeAt(i)})`);
                    break;
                }
            }
        }

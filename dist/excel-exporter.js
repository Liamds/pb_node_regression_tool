...
export class ExcelExporter {
     ...
     */
    async exportResults(results, outputFile) {
     ...
     */
    async createSheetForResult(workbook, result) {
        const sheetName = DataProcessor.sanitizeSheetName(result.formName, this.config.EXCEL_SHEET_NAME_MAX_LENGTH);
        ...
        const countDiff = DataProcessor.hasDifferences(result.variances, this.config.COLUMN_NAME_DIFFERENCE);
        // Apply tab color
     ...
     */
    async createSheetForValidationErrors(workbook, result) {
        let sheetName = DataProcessor.sanitizeSheetName(result.formName, this.config.EXCEL_SHEET_NAME_MAX_LENGTH - 18);
        sheetName = (sheetName + '_ValidationErrors').substring(0, 31);
        logger.info(`Processing Validation Errors: ${result.formName} (${result.formCode}) -> Sheet: ${sheetName}`);
        const worksheet = workbook.addWorksheet(sheetName);
     ...
     */
    async createSheetForSummary(workbook, summary) {
     ...
     */
    writeDataToSheet(worksheet, data) {
        if (data.length === 0)
            return;
     ...
     */
    writeValidationDataToSheet(worksheet, results) {
        let currentRow = 1;
     ...
     */
    writeSummaryDataToSheet(worksheet, summary) {
        ...
        summary.forEach((record) => {
            ...
            logger.info(`  Summary - Form: ${record.formName} (${record.formCode}) | Variances: ${record.varianceCount} | Validation Errors: ${record.validationErrorCount}`);
        });
        // Auto-adjust column widths
     ...
     */
    applyTabColor(worksheet, confirmed, countDiff) {
     ...
     */
    async createTable(worksheet, sheetName, data) {
        if (data.length === 0)
            return;
        ...
        worksheet.addTable({
            ...
            columns: headerArray.slice(1).map((header) => ({
     ...
     */
    async saveWorkbook(workbook, outputFile) {
        try {

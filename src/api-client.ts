...
export class AgileReporterClient {
  ...
  private readonly host = 'policebank-uat.agilereporter.com';
   ...
   */
  async getFormVersions(formCode: string): Promise<FormInstance[]> {
    if (!this.token) {
    ...
    try {
      ...
      const instances: FormInstance[] = response.data.map((record: any) => ({
   ...
   */
  async getCellMetadata(instanceId: string, cellId: string): Promise<CellMetadata> {
    if (!this.token) {
      throw new Error('Not authenticated. Call authenticate() first.');
    }
  ...
  async getCellBusinessRules(
    ...
    try {
      ...
      const businessRules: CellBusinessRule[] = response.data.map((record: any) => ({
        id: record.id,
  ...
  async getCellHeaders(
    ...
    try {
      ...
      const cellHeaders: CellRecordHeader[] = response.data.map((record: any) => ({
  ...
  async getFormAnalysis(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        logger.info(
          `Fetching variance analysis for ${formCode} (attempt ${attempt + 1}/${maxRetries})...`
        );
  ...
  async validateReturn(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        logger.info(
          `Fetching validation results for ${instance.instanceId} (attempt ${attempt + 1}/${maxRetries})...`
        );
  ...
  private parseVarianceRecords(
    ...
    variances.sort((a, b) =>
      (a['Cell Reference'] as string).localeCompare(b['Cell Reference'] as string)
    );
  ...
  private parseValidationResults(
    ...
    for (const detail of validationDetails) {
      const referencedCells: ValidationCells[] = (detail.referencedCells || []).map(
        (cellData: any) => ({
          cell: cellData.cell || '',
